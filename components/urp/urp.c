                        ...
                        const struct urp_message_handler *handler)
{
	if (handler->data_size == URP_SIZE_VARIABLE) {
		...
		data = strtol((char *)message->data_buf, &end_ptr, 10);
		...
		data = strtod((char *)message->data_buf, &end_ptr);
		if (end_ptr == (char *)message->data_buf) {
		...
		ESP_LOGE(TAG, "Incorrect data size %d for mode \"%s\"", handler->data_size, message->msg_type_str);
		return;
	} else {
		if (message->data_buf_size != handler->data_size) {
			ESP_LOGE(TAG, "Received URP data of type \"%s\" with incorrect size (%d should be %d)",
			         message->msg_type_str, message->data_buf_size, handler->data_size);
			return;
		}
	...
	ESP_LOGE(TAG, "No callback defined for %s", message->msg_type_str);
}
...
static struct urp_message_handler * handler_from_msg_type_str(struct urp_config *config, const char *msg_type_str)
{
	...
	for (int i = 0; i < config->handlers_size; i++) {
		if (strcmp(msg_type_str, config->handlers[i].msg_type_str) == 0) {
...
static int verify_checksum(struct urp_recv_message *message)
{
	...
	err = calculate_checksum(message->msg_type_str, message->data_buf,
...
static int urp_data_fill(struct urp_config *config, struct urp_recv_message *message)
{
	while (1) {
		...
		if (!message->escaped && c == URP_CHAR_START) {
			...
			message->msg_type_str[message->cursor - 1] = c;
			message->msg_type_str[message->cursor]     = '\0';
			ESP_LOGD(TAG, "msg_type_str[%d] = %c (%d)", message->cursor - 1, c, c);
...
fill:
		...
		const struct urp_message_handler *handler = handler_from_msg_type_str(config, message.msg_type_str);
		...
		memcpy(msg_type_str, message.msg_type_str, URP_VALUE_TYPE_LENGTH);
...
int urp_send_float(struct urp_config *config, const char *msg_type_str, double data)
{
	...
	data_size = sprintf(data_buf, "%f", data);
	if (data_size < 0) {
		return data_size;
	}
